...
const getUsersJoinedPost = async (postId, token, query) => {
  try {
...
const joinPost = async (postId, user) => {
  try {
    const res = await fetch(BASE_URL + "api/posts/join/" + postId, {
      method: "POST",
      headers: {
        "x-access-token": user.token,
      },
...
const unjoinPost = async (postId, user) => {
  try {
    const res = await fetch(BASE_URL + "api/posts/join/" + postId, {
      method: "DELETE",
      headers: {
